...
if [[ -s $TOP/lib/v-attest/functions.sh ]]; then
	. "$TOP/lib/v-attest/env.sh"
elif [[ -s $TOP/functions.sh ]]; then
	. "$TOP/env.sh"
else
	echo "Unable to include V-Attest ENV definitions" 1>&2
	exit 1
fi
...
v_attest_dir() {
	[[ -n $1 ]]	\
	|| die "Internal error in caller of v_attest_dir"
	case "$1" in
	bin)	echo "$TOP/bin";;
	lib)	echo "$TOP/lib";;
	etc)	if [[ $TOP = /usr ]]; then
			echo "/etc/v-attest"
		elif [[ -d $TOP/etc/v-attest ]]; then
			echo "$TOP/etc/v-attest"
		elif [[ -d $TOP/etc && -f $TOP/etc/v-attest.conf ]]; then
			echo "$TOP/etc"
		elif [[ -d $TOP && -f $TOP/v-attest.conf ]]; then
			echo "$TOP"
		elif [[ -d /etc/v-attest ]]; then
			echo "$TOP/etc"
		else
			die "Cannot find 'etc' directory for V-Attest"
		fi;;
	*)	die "Internal error in caller of v_attest_dir";;
	esac
}
v_attest_file() {
	...
	[[ -n $1 && -n $2 ]]	\
	|| die "Internal error in caller of v_attest_file"
...
function wkpriv {
	cat <<"EOF"
-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgB7hZvv39aH+h0Yg2
Cgji6Nu5YCxEud/eKOFR0cHzCMuhRANCAASK/nnAN7tf1BySWTlEmdhB5FoUSNd1
AENX+VnC9lWxhD628mwnfKsAMK3XswZyalpvaNoZZU9MHTORFtny1SLl
-----END PRIVATE KEY-----
EOF
}
