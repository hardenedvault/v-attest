...
mkdir -p /etc/v-attest
cp "${d}/sign.pem" /etc/v-attest/enroll-signer.pem
...
for i in foo bar baz; do
	...
	| V_ATTEST_DB_DIR="${d}/db"					\
	  /v-attest/sbin/attest-verify verify True			\
	...
	# attestation database (V_ATTEST_DB_DIR).
done
